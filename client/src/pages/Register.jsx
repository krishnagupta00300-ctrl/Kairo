...
export default function Register({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch('https://kairo-isfu.onrender.com/api/auth/register', {
        method: 'POST',
    ...
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <Link to="/" style={{ position: 'absolute', top: '1.5rem', left: '2rem', display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none', zIndex: 10 }}>
        <svg width="22" height="22" viewBox="0 0 100 100" fill="none">
          <defs><linearGradient id="kgr" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#9B7FFF"/><stop offset="100%" stopColor="#00CFFF"/></linearGradient></defs>
          <rect x="15" y="10" width="16" height="80" rx="4" fill="url(#kgr)"/>
          <path d="M31 50 L72 10 L90 10 L49 50Z" fill="url(#kgr)"/>
          <path d="M31 50 L72 90 L90 90 L49 50Z" fill="url(#kgr)" opacity="0.9"/>
        </svg>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', padding: '2.5rem', backdropFilter: 'blur(20px)', boxShadow: '0 0 80px rgba(155,127,255,0.08)' }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.88rem', marginBottom: '2rem', textAlign: 'center' }}>
              Start organizing your day with AI
            </p>
          </motion.div>
          ...
          <form onSubmit={handleSubmit}>
            {[
              { label: 'Your name', key: 'name', type: 'text', placeholder: 'Krishna' },
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.75 }}
            ...
            <Link to="/login" style={{ color: '#9B7FFF', textDecoration: 'none', fontWeight: 500 }}>Log in</Link>
          </motion.div>
        </div>
      </motion.div>
    </div>
